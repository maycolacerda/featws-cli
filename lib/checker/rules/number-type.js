...
const verifyFeatureRule = async (parsed, featureRule) => {
    ...
    switch (operator) {
        ...
        case '<', '>', '>=', '<=':
            if(leftparam!==null && leftparam.type!=='integer' && leftparam.type!=='decimal'){
                if (lefttype=='#'){
                    return `parameter '${leftvalue}' isn't an 'integer' or 'decimal'!`;
                }
                else{
                    return `feature '${leftvalue}' isn't an 'integer' or 'decimal'!`;
                }
            }
            if(rightparam!==null && rightparam.type!=='integer' && rightparam.type!=='decimal'){
                if (righttype=='#'){
                    return `parameter '${rightvalue}' isn't an 'integer' or 'decimal'!`;
                }
                else{
                    return `feature '${rightvalue}' isn't an 'integer' or 'decimal'!`;
                }
            }
            ...
            break;
        case '==', '!=':
            if (leftparam!==null && leftparam.type==='string' && rightparam!==null && rightparam.type!=='string'){
                if (righttype=='#'){
                    return `parameter '${rightvalue}' isn't a string!`;
                }
                else{
                    return `feature '${rightvalue}' isn't a string!`;
                }
            if (leftparam!==null && leftparam.type!=='string' && rightparam!==null && rightparam.type==='string'){
                if (lefttype=='#'){
                    return `parameter '${leftvalue}' isn't a string!`;
                }
                else{
                    return `feature '${leftvalue}' isn't a string!`;
                }
            }
            break;
